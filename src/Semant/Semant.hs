...
match lty rty tenv pos = do
        ...
        if lty' == rty' || T.NIL == lty' || T.NIL == rty'
                then return ()
                else Err.returnErr "" (Err.TypeMismatch (show lty) (show rty)) pos
    where
...
transVar st@(SS venv tenv) = trvar
    where
        ...
        trvar (A.SimpleVar sym pos) = case S.look venv sym of
                Nothing -> Err.returnErr "" (Err.UnknownIdentifier sym) pos
                Just (E.VarEntry ty) -> return $ ExpTy () ty
                Just (E.FunEntry{}) -> Err.returnErr "" (Err.UnknownIdentifier sym) pos
        trvar (A.FieldVar v sym pos) = do
                ty <- ty <$> trvar v
                case ty of
                        T.RECORD _ fs -> case lookup sym fs of
                                Nothing -> Err.returnErr "" (Err.RecordFieldNotFound sym) pos
                                Just ty' -> return $ ExpTy () ty'
                        _ -> Err.returnErr "" (Err.WrongType "RECORD" (show v)) pos
        trvar (A.SubscriptVar v exp pos) = do
                ...
                case t of
                        T.ARRAY _ ty' -> do
                                ...
                                return $ ExpTy () ty'
                        _ -> Err.returnErr "" (Err.WrongType "ARRAY" (show v)) pos
...
transExp st@(SS venv tenv) = trexp
    where
        ...
        trexp (A.CallExp fun args pos) = case S.look venv fun of
                Nothing -> Err.returnErr "" (Err.UnknownIdentifier fun) pos
                Just (E.VarEntry _) -> Err.returnErr "" (Err.UnknownIdentifier fun) pos
        ...
        trexp (A.OpExp left op right pos) = do
                ...
                if case (lty, rty) of
                        ...
                        else Err.returnErr "" (Err.InvalidComparison (show lty) (show rty)) pos
                ...
                Just ty' -> do
                    ...
                    where
                        ...
                        isRecord ty = case ty of
                                ...
                                _ -> Err.returnErr "" (Err.WrongType "RECORD" (show ty)) pos
                        ...
                        checkrecord fs ((s, t) : sts) = case lookup s fs of
                                Just (e, p) -> do
                                        ...
                                        match t ty tenv pos
                                        checkrecord fs sts
                                Nothing -> Err.returnErr "" (Err.RecordFieldNotFound s) pos
        ...
        trexp (A.ArrayExp typ size init pos) = case S.look tenv typ of
            ...
            where
                ...
                isArray ty = case ty of
                        ...
                        _ -> Err.returnErr "" (Err.WrongType "ARRAY" (show ty)) pos
            ...
            where
                ...
                registy' header decs st@(SS venv tenv) = case decs of
                        ...
                        A.TypeDec name ty pos : ds -> do
                                when (isJust $ S.look tenv name) (Err.returnErr "" (Err.MultipleDeclarations name) pos)
            ...
            where
                ...
                regisfun' header decs st@(SS venv tenv) = case decs of
                        ...
                        A.FunDec name params result body pos : ds -> do
                                when (isJust $ S.look venv name) (Err.returnErr "" (Err.MultipleDeclarations name) pos)
...
transDecs decs = evalStateT $ do
    ...
    where
        ...
        transFunDec (A.FunDec name params result body pos) = StateT $ \st@(SS venv tenv) -> do
                ...
                let fun = E.FunEntry (map (\(_, x, _) -> x) params') result_ty
                    venv' = S.enter venv name fun
                    (_, st') = runState (mapM enterparam params') st{venv = venv'}
                ty <- ty <$> transExp st' body
                match result_ty ty tenv pos
                return ((), st{venv = venv'})
